...
class ProjectCLDDataService
{
    // Cache frequently used field names
    protected const DIAGNOSIS_FIELD = 'liver_main_diag';
    ...
    protected const STATUS_FIELDS = [
        ...
        'age_demo',
        'enroll_date_demo',
        'liver_enrollment_date',
        'liver_next_appo_date',
        'liver_outcome',
        'liver_outcome_date'
    ];
    ...
    public function getRecentCLDDeaths(int $projectId, string $condition): Collection
    {
        $cldPatients = $this->getAllRespondents($projectId, $condition);

        /** @var Collection $cldDeaths */
        $cldDeaths = app(ProjectDataService::class)->getCLDDeathRecords($projectId);

        $threeMonthsAgo = now()->subMonths(3);

        return $cldPatients->filter(function ($patient) use ($cldDeaths, $threeMonthsAgo) {
            $deathRecord = $cldDeaths->firstWhere('record', $patient['record']);
    ...
    private function processRecords(Collection $records): Collection
    {
        return $records->groupBy('record')
            ->map(function (Collection $recordItems, string $recordId) {
                ...
                $dmOutcome = $this->getFieldValue($latestInstance, 'liver_outcome');
                $latestOutcomeDate = $this->getFieldValue($latestInstance, 'liver_outcome_date');
                $nextAppoDate = $this->getFieldValue($latestInstance, 'liver_next_appo_date');
                $dmEnrollDate = $this->getFieldValue($groupedByInstance->get(1), 'liver_enrollment_date');
                ...
                return [
                    ...
                    'liver_next_appo_date' => $nextAppoDate,
                    'liver_enrollment_date' => $dmEnrollDate,
                    'latest_outcome_date' => $latestOutcomeDate,
                    'liver_outcome' => $dmOutcome,
    ...
    /**
     * Determine respondent status based on liver_outcome and appointment date
     */
